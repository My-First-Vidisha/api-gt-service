pipeline {
   ...
   stages {
      stage('Preparation') {
         steps {
            cleanWs()
             git credentialsId: 'Myajenkins', url: "https://github.com/My-First-Vidisha/api-gt-service"
		  sh 'ls -l'
		  sh 'cat Dockerfile'
		sh 'echo ${WORKSPACE}'
         }
      ...
      stage("Build image") {
       ...
       stage('Deploy to GKE') {
            steps{
                sh "sed -i 's/backend-api:latest/backend-api:${env.BUILD_ID}/g' backend.yaml"
